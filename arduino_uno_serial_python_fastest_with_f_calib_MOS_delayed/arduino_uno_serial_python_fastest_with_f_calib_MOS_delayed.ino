...
float conversionF=0.26*1.25; //50 steps give 13 mm of move
#define jmax  60 //57   
 ...
 Serial.println(" Sending a pulse now!"); 
       ...
       // analogWrite(TransistorBasePin,0);
         ...
         for (int i=0; i<maxData; i++) {
            ...
            if(i>20){delayMicroseconds(500);cumulTime=cumulTime+0.5;};
            if(i>30){delay(2);cumulTime=cumulTime+2;};
           // BiasTrig[i]=cumulTime;
           if(cumulTime>20){digitalWrite(biasTrigger, LOW); BiasTrig[i]=1; }else{BiasTrig[i]=0;}
