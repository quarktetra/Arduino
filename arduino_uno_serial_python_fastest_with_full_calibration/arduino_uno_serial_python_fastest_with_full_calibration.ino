...
void loop() {
  ...
  if(TriggerButtonPinValue==0){  ;triggered=1;};
    ...
    //Serial.print(" IN the main loop TriggerButtonPinValue:");Serial.print(TriggerButtonPinValue);Serial.print(", triggered:");Serial.println(triggered); 
delay(100);
...
if(CentralCailbrationDone==0){        
 ...
 delay(1000);
    ...
    initOV=analogRead(OpticalSensorPin);
     ...
     delay(200);
     digitalWrite(StatusLEDPin,LOW);
 analogWrite(TransistorBasePin,255);delay(3000); 
...
for (i = 0; i<sweepStepsPerRev; i++)       // Iterate for 4000 microsteps. 
 ...
 FullCailbrationDone=1;analogWrite(TransistorBasePin,0);
